    ...
    public partial class Frm_US_UuDai : UserControl
    {
        ...
        public void loadGird(string search)
        {
            ...
            foreach (var x in _service.GetUudais(search))
            {
                ...
                dtgHienthi.Rows.Add(stt++, x.Mauudai, x.Tenuudai, idtk.Hovaten, x.Phantram, x.Soluong, x.Ngaybatdau, x.Ngayketthuc, MapTrangThai(x.Trangthai));
            }
        }
        ...
        {
            if (ngayKetThuc <= ngayHienTai && ngayBatDau <= ngayKetThuc)
                return "Kết thúc";
